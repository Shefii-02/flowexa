...
const TABS = ['Attendance', 'Payroll', 'Incentives', 'Leave', 'Break types', 'Leave types', 'Staff setup', 'Office'] as const
...
function LeaveApprovalTab() {
  const [rows, setRows] = useState<any[]>([])
  const load = useCallback(() => { api.get('/hr/leave', { params: { scope: 'team' } }).then(r => setRows(r.data?.data ?? [])).catch(() => {}) }, [])
  useEffect(() => { load() }, [load])
...
function StaffSetupTab() {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl overflow-x-auto">
      <table className="w-full text-sm">
        <thead><tr className="text-left text-gray-400 border-b border-gray-100">
          <th className="p-2">Staff</th><th className="p-2">Check-in</th><th className="p-2">Work mode</th><th className="p-2">Duty start</th><th className="p-2">Duty end</th><th className="p-2">Hourly rate</th><th className="p-2">Monthly salary</th>
        </tr></thead>
        <tbody>
          {rows.map(r => (
