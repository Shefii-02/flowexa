...
// A second source tab shows backend-node's own audit trail, proxied live.
...
export default function ApiLogsPage() {
  ...
  const filters = useCallback(() => ({
    ...
    method: !isNode && method ? method : undefined,
    activity_only: !isNode && activityOnly ? activityOnly : undefined,
    is_error: !isNode && errorOnly ? '1' : undefined,
    search: !isNode && search ? search : undefined,
    page,
    per_page: 30,
  }), [source, isNode, companyId, method, activityOnly, errorOnly, search, page])

  const load = useCallback(() => {
    setLoading(true)
    Promise.all([
      superadminApi.apiLogs(filters()),
      superadminApi.apiLogStats({ source, company_id: !isNode && companyId ? companyId : undefined, days: 7 }),
    ]).then(([logs, s]) => {
      setRows(logs.data.data ?? [])
      setTotal(logs.data.total ?? 0)
      setStats(s.data)
    }).catch((e) => toast.error(getError(e))).finally(() => setLoading(false))
  }, [filters, source, isNode, companyId])
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="page-sub">{isNode ? "Backend-node's own audit trail — live from the gateway, nothing stored here." : 'Every Laravel API request — request volume, latency, error rate, and a human activity feed.'}</p>
      </div>
      ...
      {!isNode && stats?.top_companies?.length > 0 && !companyId && (
        <div className="card">
            ...
            <table className="table">
              ...
              <tbody>
                {stats.top_companies.map((c: any) => (
                  <tr key={c.company_id}>
                    <td><button className="text-brand-600 hover:underline" onClick={() => setParams({ source, company_id: String(c.company_id) })}>{c.company ?? `#${c.company_id}`}</button></td>
        ...
        {loading ? <TableSkeleton rows={10} cols={7} /> : rows.length === 0 ? (
          ...
          <>
            <div className="table-wrapper">
              <table className="table">
                <thead>
                  <tr><th>Time</th>{!isNode && <th>Company</th>}<th>Actor</th><th>Method</th><th>Path</th><th>Status</th>{!isNode && <th>Duration</th>}</tr>
                </thead>
                <tbody>
                  {rows.map((r) => (
                    <tr key={r.id} className="cursor-pointer" onClick={() => setDetail(r)}>
                      ...
                      {!isNode && <td className="text-xs">{r.duration_ms} ms</td>}
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <Modal open={!!detail} onClose={() => setDetail(null)} title="Request detail" size="lg">
        {detail && (
          <div className="space-y-3 text-sm">
            ...
            <p><span className="text-gray-400">Status:</span> {detail.status_code}{!isNode ? ` · ${detail.duration_ms} ms` : ''}</p>
            <p><span className="text-gray-400">Actor:</span> {detail.user?.name ?? '—'} {detail.actor_role ? `(${detail.actor_role})` : ''} {detail.ip ? `· ${detail.ip}` : ''}</p>
