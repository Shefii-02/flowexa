    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <div className="card">
        ...
        <div className="card-body space-y-4">
          ...
          {/* ── Verify result panel ── */}
          {verifyResult && (
            <div className={`rounded-xl border p-4 text-sm space-y-2 ${verifyResult.connected
              ...
              {verifyResult.connected && verifyResult.phone_number && (
                <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-green-700 mt-2">
                  ...
                  <span className={`font-semibold ${verifyResult.quality_rating === 'GREEN' ? 'text-green-600' :
          ...
          {loadingLogs ? (
            ...
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {webhookLogs.map((log: any) => (
                <div key={log.id} className={`rounded-lg border px-3 py-2 text-xs font-mono ${log.status === 'received' ? 'bg-blue-50 border-blue-200' :
                    log.status === 'processed' ? 'bg-green-50 border-green-200' :
                      log.status === 'error' ? 'bg-red-50 border-red-200' :
                        'bg-gray-50 border-gray-200'
                  }`}>
                  <div className="flex items-center justify-between mb-1">
                    <span className={`font-semibold ${log.status === 'received' ? 'text-blue-700' :
